...
public class ElfComputer {
    ...
    public List<Long> executeProgram() {
        ...
        while(!haltExecution) {
            long[] instructionsAndParameters = determineOpcodeAndParameters(SystemIndex);

            //System.out.println("Opcode: " + instructionsAndParameters[0]);
            switch (Math.toIntExact(instructionsAndParameters[0])) {
                case 1: //addition
                ...
                case 5: //jump-if-true
                    if (instructionsAndParameters[1] != 0) {
                        SystemIndex = Math.toIntExact(instructionsAndParameters[2]);
                    }
                    else {
                        SystemIndex += 3;
                    }
...
//        }
        ...
        for (int i = 1; i <= parameterNumber; i++) {
            ...
            if (parameters[i] == 0) {
                ...
                if (parameters[0] == 3 || i == 3) {
                    parameters[i] = programMemory[index + i] + relativeBase;
                }
                else {
                ...
                throw new IllegalArgumentException("Invalid parameter mode detected");
            }
        }
        return parameters;
    }
